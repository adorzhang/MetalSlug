...
void CLegs::Render()
{
	...
	ani = LEG_ANI_WALK; //Animation chỉ có 1 frame duy nhất
	if (vx == 0) {
		animation_set->at(ani)->isPause = true; //Dừng animation 
		animation_set->at(ani)->Render(x + MAIN_CHARACTER_BBOX_WIDTH - LEG_BBOX_WIDTH + x_delta, y - MAIN_CHARACTER_BBOX_HEIGHT, flip, alpha);
	}
	else // Nhân vật di chuyển
	{
		animation_set->at(ani)->isPause = false; // Tiếp tục animation đã dừng trước đó
		animation_set->at(ani)->Render(x + MAIN_CHARACTER_BBOX_WIDTH - LEG_BBOX_WIDTH + x_delta, y - MAIN_CHARACTER_BBOX_HEIGHT  + y_delta, flip, alpha);
	}

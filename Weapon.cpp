	...
	if (state != WEAPON_STATE_NONE)
	{
		...
		if (type_weapon == WEAPON_TYPE_PLAYER)
		{
			for (UINT i = 0; i < colliable_object->size(); i++)
			{
				...
				else if (dynamic_cast<CLocust*>(colliable_object->at(i)))
				{
					CLocust* locust = dynamic_cast<CLocust*>(colliable_object->at(i));
					if (locust->GetState() != STATE_ITEM)
					{
						float l1, t1, r1, b1, l2, t2, r2, b2;
						GetBoundingBox(l1, t1, r1, b1);
						locust->GetBoundingBox(l2, t2, r2, b2);
		...
		else if (type_weapon == WEAPON_TYPE_PLAYER_ROCKET)
		{
			...
			if (minObjectDistance)
			{
				...
				else if (dynamic_cast<CLocust*>(minObjectDistance))
				{
					CLocust* locust = dynamic_cast<CLocust*>(minObjectDistance);
					if (locust->GetState() != STATE_ITEM)
					{
						float l1, t1, r1, b1, l2, t2, r2, b2;
						GetBoundingBox(l1, t1, r1, b1);
